...
Given qr{^a HTTP server that( supports Range requests and)?(| does not) serve[s]? "([^"]+)" in "([^"]+)"(?: with (content|size) "?([^"]*)"?)?}, fun ($c) {
    ...
    if ($range_req) {
        $c->{stash}->{scenario}->{test_resume} = 1;
        ...
        systemx(
            ...
            '--quiet',
            # XXX: enable this once we require Buster or newer for running
            # this test suite
            # '--collect',
...
When qr{^I download "([^"]+)" \(of expected size (\d+)\) from "([^"]+)", and check its hash is "([^"]+)"$}, fun ($c) {
    ...
    if ($c->{stash}->{scenario}->{test_resume}) {
...
After fun ($c) {
    ...
    # XXX: remove once we use --collect for all our transient units
    systemx('systemctl', '--user', 'reset-failed');
};
